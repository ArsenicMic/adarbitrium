...
@WebServlet("/DeclinaisonsAdjectifs")
public class DeclinaisonsAdjectifs extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public DeclinaisonsAdjectifs() {
        super();
    }


	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("DECLINAISON-ADJ");
		...
		if (session == null) {
			...
			System.out.println("declinaison-adj-registered");
			QuestionGenerator generator;
			if (session.getAttribute("questionGenerator") == null) {
				Vocabulaire vocabulaire = new Vocabulaire(Mot.Mot_type.TYPE_ADJECTIF, new ArrayList<Integer>(Arrays.asList(1, 2)));
				ArrayList<Adjectif> voc = (ArrayList<Adjectif>) vocabulaire.getVocListe();
				generator = new QuestionGenerator(QuestionGenerator.mQuestionType.TYPE_QUESTION_CAS_GENRE_NOMBRE, voc);
			...
			this.getServletContext().getRequestDispatcher("/WEB-INF/declinaisonsAdjectifs.jsp").forward(request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("hasCatChoice") != null) {
			...
			Vocabulaire vocabulaire = new Vocabulaire(Mot.Mot_type.TYPE_ADJECTIF, chosenCategories);
			ArrayList<Adjectif> voc = (ArrayList<Adjectif>) vocabulaire.getVocListe();
			generator = new QuestionGenerator(QuestionGenerator.mQuestionType.TYPE_QUESTION_CAS_GENRE_NOMBRE, voc);			
			session.setAttribute("questionGenerator", generator);
			Question q = generator.generate();
			...
			request.setAttribute("question", q);
			this.getServletContext().getRequestDispatcher("/WEB-INF/new-question-cas-genre-nb.jsp").forward(request, response);
		}
	}
